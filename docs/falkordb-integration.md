...
- 「发哥部署在哪台服务器？」——向量库里是两条独立记忆（`发哥` 和 `claw-1-164`），没有边连接
- 图存储把实体抽成节点、关系抽成边：`(发哥)-[部署于]->(claw-1-164)`
...
上游的冲突消解是**破坏性**的：LLM 判定旧关系与新信息矛盾后直接物理删除——误判即永久丢失。

本 Fork 改为**失效保留**：

| 行为 | 上游 | 本 Fork |
|------|------|---------|
| 冲突消解 | `DELETE r` 物理删除 | 写入 `invalidated_at` 标记失效 |
| 检索 | — | 默认只返回有效关系（`invalidated_at IS NULL`） |
| 同事实重现 | 无 | 自动复活（MERGE 命中失效边时重置标记） |
| 存量数据 | — | 无标记视为有效，向后兼容 |
